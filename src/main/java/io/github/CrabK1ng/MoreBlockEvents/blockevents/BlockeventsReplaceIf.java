...
public class BlockeventsReplaceIf extends PuzzleBlockAction {

    String blockStateId;
    String ReplaceBlockStateId;
    int xOff;
    int yOff;
    int zOff;
    ...
    protected void act(BlockState srcBlockState, Zone zone, @Nullable BlockPosition sourcePos, Map<String, Object> map) {
        ...
        this.ReplaceBlockStateId = getParameters().get("ReplaceBlockStateId").asString();
        ...
        if (bp != null) {
            if (bp.getBlockState() == BlockState.getInstance(ReplaceBlockStateId)){
